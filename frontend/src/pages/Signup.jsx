  ...
  return (
    <div className="min-h-screen min-w-[100vw] flex items-center justify-center bg-[linear-gradient(135deg,#e8eef7_0%,#dfe7f2_50%,#f0f4f9_100%)] p-5 md:p-10 box-border font-sans text-[#3b3f4a] relative overflow-hidden">
      ...
      >
        ...
        <h2 id="signup-heading" className="text-center mb-2 mt-0 text-[28px] md:text-[28px] font-semibold text-[#4a505d] tracking-[-0.5px]">Sign Up</h2>
        <div className="text-center text-[14px] text-[#9ba3b0] mb-8 font-normal">Create your account below</div>

        <form aria-label="Sign up form" onSubmit={handleSubmit}>
          <div className="flex flex-col mb-5">
            <label className="text-[13px] font-medium text-[#6b7280] mb-2 flex items-center gap-1.5" htmlFor="fullname">
              <span className="text-[14px] opacity-60">👤</span> Full Name
            </label>
            <input 
              id="fullname" 
              name="fullname" 
          ...
          {status && (
            <div className={`mt-3 text-[13px] font-semibold text-[#4a5568] ${(status.toLowerCase().includes("fail") || status.toLowerCase().includes("error") || status.toLowerCase().includes("already") ? "text-[#d14343]" : "")}`}>
              {status}
            </div>
          )}
        </form>
        ...
        <div className="mt-6 text-center text-[13px] text-[#9ba3b0]">
          ...
          <Link to="/signin" className="text-[#7b8cd9] no-underline font-medium ml-1 hover:text-[#6a7bc5]">Sign in</Link>
        </div>
      </div>
    </div>
  );
